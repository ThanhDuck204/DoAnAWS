      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-black text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
              AI Generated - {formatConfidenceText(evidence)}
            </span>
            {needsReview && (
              <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-black text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
                <FiAlertTriangle className="h-3 w-3" />
                Needs review
              </span>
            )}
            {needsConfirmation && (
              <span className="rounded-full bg-rose-100 px-2.5 py-1 text-[10px] font-black text-rose-700 dark:bg-rose-900/30 dark:text-rose-400">
                Need confirmation
              </span>
            )}
            {deadlineWarning && (
              <span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${deadlineWarning.tone === 'red' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'}`}>
                {deadlineWarning.label}
              </span>
            )}
            {workload?.overloaded ? (
              <span className="rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-black text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
                This member may be overloaded
              </span>
            ) : null}
            {task.possibleDuplicate ? (
              <span className="inline-flex items-center gap-1 rounded-full bg-violet-100 px-2.5 py-1 text-[10px] font-black text-violet-700 dark:bg-violet-900/30 dark:text-violet-400">
          ...
          <input
            ...
            className="w-full rounded-lg border border-transparent bg-transparent text-sm font-black text-slate-900 outline-none focus:border-blue-200 focus:bg-blue-50 focus:px-2 focus:py-1 dark:text-slate-100 dark:focus:border-blue-700 dark:focus:bg-blue-900/20"
          />
          <textarea
          ...
          <div className="grid gap-2 md:grid-cols-4">
            <select
              ...
              className="rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs font-bold text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400"
            >
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <button
                  ...
                  className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-[11px] font-black text-slate-500 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-400 dark:hover:bg-slate-800"
                >
                  {label}
                </button>
              ))}
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800">
            <button
              ...
              className="flex w-full items-center justify-between px-3 py-2 text-left text-xs font-black text-slate-700 dark:text-slate-300"
            >
            ...
            {evidenceOpen ? (
              <div className="border-t border-slate-200 px-3 py-3 text-xs leading-5 text-slate-600 dark:border-slate-700 dark:text-slate-400">
                ...
                <blockquote className="mt-2 rounded-lg border-l-4 border-blue-300 bg-white px-3 py-2 text-slate-600 dark:border-blue-700 dark:bg-slate-800 dark:text-slate-400">
