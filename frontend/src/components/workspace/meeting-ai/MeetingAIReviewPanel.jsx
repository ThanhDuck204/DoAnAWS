    ...
    <div className="space-y-5">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900/80">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h2 className="mt-3 text-2xl font-black text-slate-950 dark:text-slate-100">{meeting.title}</h2>
            <div className="mt-2 flex flex-wrap gap-3 text-xs font-bold text-slate-400 dark:text-slate-500">
          ...
          {meeting.status === 'AI_REVIEW_READY' && canManageMeetings && (
            <button
              ...
              className="rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-black text-white shadow-lg shadow-slate-900/20 transition hover:bg-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700"
            >
          ...
          {['overview', 'timeline', 'transcript', 'summary', 'suggested', 'generated'].map((section) => (
            <button
              ...
              className={`rounded-full px-3.5 py-2 text-xs font-black capitalize transition ${
                activeSection === section ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700'
              }`}
            >
      ...
      {activeSection === 'transcript' && (
        <Panel title="Transcript" icon={FiFileText}>
          <pre className="max-h-[520px] overflow-y-auto whitespace-pre-wrap rounded-xl bg-slate-50 p-4 text-sm leading-7 text-slate-600 dark:bg-slate-800 dark:text-slate-400">{visibleTranscript || 'No transcript available.'}</pre>
