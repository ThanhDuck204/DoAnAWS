    ...
    <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
      ...
      >
        <div className="mb-6">
          ...
          <h2 className="mt-1 text-2xl font-bold text-slate-950">Upload audio or transcript</h2>
        ...
        <div className="space-y-5">
          <div>
            ...
            <input
              ...
              className="h-11 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none transition focus:border-primary-500 focus:ring-4 focus:ring-primary-100"
            />
          </div>
          ...
          <div>
            <label className="mb-2 block text-sm font-semibold text-slate-700">Audio file</label>
            <label className="group relative flex min-h-[150px] cursor-pointer flex-col items-center justify-center overflow-hidden rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-5 py-6 text-center transition hover:-translate-y-1 hover:border-primary-400 hover:bg-primary-50/40 hover:shadow-lg hover:shadow-blue-100">
          ...
          <div>
            <label className="mb-2 block text-sm font-semibold text-slate-700">Transcript</label>
            <textarea
              value={formData.transcript}
              onChange={(event) => setFormData((current) => ({ ...current, transcript: event.target.value }))}
              rows={8}
              className="w-full rounded-xl border border-slate-200 px-3 py-3 text-sm leading-6 outline-none transition focus:border-primary-500 focus:ring-4 focus:ring-primary-100"
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 rounded-xl border px-3 py-2 text-left transition ${
                      selected ? 'border-primary-200 bg-primary-50 text-primary-700' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50'
                    }`}
                  >
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="block text-xs text-slate-500">{participant.role}</span>
                    </span>
                  </button>
                );
              })}
      ...
      >
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-bold text-slate-950">Summary and tasks</h2>
          </div>
        ...
        {!result && !processing && (
          <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center">
            ...
            <h3 className="mt-4 text-lg font-bold text-slate-900">No AI result yet</h3>
            <p className="mt-2 text-sm leading-6 text-slate-500">
        ...
        {result && !processing && (
          <div className="space-y-5">
            ...
            <div>
              <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-500">Key discussion points</h3>
              <div className="space-y-2">
                {result.keyDiscussionPoints.map((point) => (
                  <div key={point} className="flex items-start gap-3 rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-700">
