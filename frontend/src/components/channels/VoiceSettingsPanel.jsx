    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4">
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-xl bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-base font-black text-slate-900">Voice input settings</h3>
            <p className="mt-1 text-xs font-semibold text-slate-400">Microphone, input volume, sensitivity, processing, and live test.</p>
          </div>
        ...
        <div className="mt-5 grid gap-4">
          ...
          <label className="grid gap-2 text-xs font-black uppercase tracking-wide text-slate-500">
            ...
            <select
              ...
              className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold normal-case text-slate-700"
            >
          ...
          <div className="grid gap-2 rounded-xl border border-slate-200 p-4">
            <span className="text-xs font-black uppercase tracking-wide text-slate-500">Presets</span>
          ...
          <p className="-mt-2 text-[11px] leading-5 text-slate-400">Lower value = more sensitive. Higher value = less sensitive.</p>
  ...
  return (
    <div className="rounded-xl border border-slate-200 px-4 py-3">
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-lg bg-slate-100 px-2 py-0.5 text-xs font-black text-slate-600">{value}{suffix}</span>
      </div>
      <input
        type="range"
        min={min}
        ...
        className="mt-3 h-1.5 w-full cursor-pointer appearance-none rounded-full bg-slate-200 accent-emerald-500"
      />
    </div>
  );
}
...
function ToggleRow({ icon, label, hint, checked, onChange }) {
  return (
    <label className="flex items-center justify-between rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold text-slate-700">
      ...
      <button type="button" role="switch" aria-checked={checked} onClick={onChange} className={`relative h-6 w-11 rounded-full transition-colors ${checked ? 'bg-emerald-500' : 'bg-slate-300'}`}>
